                ...
                Select::make('type')
                    ...
                    ->afterStateUpdated(function ($state, Set $set, $livewire) {
                        ...
                        if ($state === 'lost') {
                            ...
                            $set('notes', 'Charge 100 % of purchase price due to asset loss.');
                        } elseif ($state === 'late') {
                            if (!$ticket->due_at) {
                                return;
                            }
                            ...
                            $returned = $record->returned_at ? Carbon::parse($record->returned_at)->startOfDay() : now()->startOfDay();

                            if ($returned->gt($due)) {
                                $days = $due->diffInDays($returned);
                                $finePerDay = 10000;

                                $set('amount', $days * $finePerDay);
                                $set('notes', "Late return by {$days} Days. Rate: (IDR " . number_format($finePerDay, 0, ',', '.') . " /Day.)");
                            } else {
                                $set('amount', 0);
